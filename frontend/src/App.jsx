      ...
      {entries.map(([name, src], i) => {
        ...
        return (
          <g key={name}>
            ...
            {s > 1 && <rect x={labelW} y={y} width={w} height={barH} rx={compact ? 5 : 7} fill={col} opacity={0.9} style={{ filter: `drop-shadow(0 0 4px ${col})` }}/>}
            {s <= 1 && <rect x={labelW} y={y} width={barMaxW * 0.08} height={barH} rx={compact ? 5 : 7} fill="#10B981" opacity={0.5}/>}
            <text x={labelW + barMaxW + 6} y={y + barH / 2 + 4} fontSize={fSizeV} fontWeight="700" fill="#F9FAFB">
              {s <= 1 ? (s === 1 && name === "GSB" ? (src?.checked === false ? "N/A" : src?.flagged ? "Flagged" : "Clean") : "Clean") : s}
            </text>
          </g>
        );
      })}
...
<div class="page">
  ...
  <div class="ftr"><span>ThreatLattice</span><span>${now}</span></div>
</div></body></html>`;
...
function ReportPage({ ioc, iocType, data, onBack }) {
  ...
  const pieData = Object.entries(d.sources).map(([name, src]) => {
    ...
    const isClean = val <= 1;
    return { label: name, value: isClean ? 5 : val, color: isClean ? "#10B981" : (SOURCE_COLORS[name] || "#888") };
  });
                ...
                {Object.entries(d.sources).map(([name, src]) => {
                  ...
                  return (
                    <div key={name} style={{ display: "flex", alignItems: "center", gap: 7 }}>
                      ...
                      <span style={{ fontSize: 12, fontWeight: 700, color: "#F9FAFB", marginLeft: "auto" }}>{s <= 1 ? "Clean" : s}</span>
                    </div>
                  );
                })}
              </div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 32 }}>
      {items.map(f => {
        ...
        return (
          <div
            key={f.title}
            onClick={() => setActive(isActive ? null : f.title)}
            style={{
              background: isActive ? "#1a2332" : "#111827",
              border: `1px solid ${isActive ? f.color : "#1f2937"}`,
              borderRadius: 14,
              padding: "18px 14px",
              cursor: "pointer",
          ...
          >
            ...
            <div style={{ fontSize: 24, fontWeight: 900, color: f.color, marginBottom: 2, textShadow: `0 0 16px ${f.color}66` }}>{f.stat}</div>
            <div style={{ fontSize: 10, color: "#6B7280", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 8 }}>{f.statLabel}</div>
            <div style={{ fontSize: 11, fontWeight: 700, color: isActive ? f.color : "#9CA3AF", marginBottom: 4, lineHeight: 1.3 }}>{f.title}</div>
            <div style={{ fontSize: 10, color: "#4B5563" }}>click for details {isActive ? "▴" : "▾"}</div>
            {isActive && (
              <div style={{ fontSize: 11, color: "#9CA3AF", lineHeight: 1.7, borderTop: `1px solid ${f.color}33`, paddingTop: 10, marginTop: 10, textAlign: "left" }}>
                {f.desc}
              </div>
            )}
          </div>
    ...
    <div style={{ minHeight: "100vh", background: "#0a0f1a", fontFamily: "system-ui, sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1200, margin: "0 auto", padding: "40px 24px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 48 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{ width: 48, height: 48, borderRadius: 14, background: "linear-gradient(135deg, #1e3a5f, #3B82F6)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 24px #3B82F655" }}>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                <polygon points="12,2 20,7 20,17 12,22 4,17 4,7" stroke="white" strokeWidth="1.5" fill="none"/>
                <polygon points="12,7 16.5,9.5 16.5,14.5 12,17 7.5,14.5 7.5,9.5" stroke="#60A5FA" strokeWidth="1" fill="none"/>
                <circle cx="12" cy="12" r="2" fill="white"/>
              </svg>
            </div>
            <div>
              <div style={{ fontSize: 20, fontWeight: 900, color: "#F9FAFB", letterSpacing: "-0.02em" }}>ThreatLattice</div>
              <div style={{ fontSize: 11, color: "#4B5563", fontWeight: 600, letterSpacing: "0.08em" }}>THREAT INTELLIGENCE PLATFORM</div>
            </div>
          </div>
        ...
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <h1 style={{ fontSize: 48, fontWeight: 900, color: "#F9FAFB", marginBottom: 14, lineHeight: 1.05, letterSpacing: "-0.03em" }}>
            Unified Threat<br/>
        ...
        {/* Feature cards left, Stats right */}
        <FeatureAndStats />
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "center", marginBottom: 40 }}>
          {IOC_TYPES.map(t => (
            <button key={t.id}
              ...
              style={{ padding: "14px 28px", borderRadius: 28, background: t.bg, border: `1px solid ${t.color}55`, fontSize: 15, color: t.text, fontWeight: 800, cursor: "pointer", transition: "all .12s", letterSpacing: "0.02em" }}
            ...
            {FEED_CATS.filter(c => feedLoading[c.id]).map(cat => (
              <div key={cat.id} style={{ background: "#0d1117", borderRadius: 12, padding: "16px 18px", border: `1px solid ${cat.color}22`, marginBottom: 10 }}>
                <div style={{ display: "flex", gap: 8, marginBottom: 10, alignItems: "center" }}>
                  ...
                  <div style={{ width: 80, height: 16, background: "#1f2937", borderRadius: 8 }}/>
                </div>
                <div style={{ height: 13, background: "#1f2937", borderRadius: 6, marginBottom: 7, width: "80%" }}/>
                <div style={{ height: 11, background: "#1f2937", borderRadius: 6, width: "60%" }}/>
              </div>
            ))}
            {feed.length > 0 && (
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 14px", display: "flex", flexDirection: "column", gap: 10 }}>
        {messages.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
            <div style={{ maxWidth: "88%", padding: "9px 13px", borderRadius: m.role === "user" ? "14px 14px 4px 14px" : "4px 14px 14px 14px", background: m.role === "user" ? "linear-gradient(135deg, #2563EB, #3B82F6)" : "#1f2937", border: m.role === "user" ? "none" : "1px solid #374151" }}>
              {m.role === "user" ? <div style={{ fontSize: 13, color: "white", lineHeight: 1.5 }}>{m.content}</div> : <div>{renderMsg(m.content)}</div>}
            </div>
          </div>
        ))}
        {loading && <div style={{ display: "flex" }}><div style={{ padding: "9px 13px", background: "#1f2937", borderRadius: "4px 14px 14px 14px", border: "1px solid #374151", display: "flex", gap: 4, alignItems: "center" }}>{[0,1,2].map(i => <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: "#3B82F6" }}/>)}</div></div>}
...
function MainApp({ onLogout }) {
  ...
  const loadCategory = async (catId) => {
    ...
    try {
      ...
      if (res.status === 429) { setFeedErrors(prev => ({ ...prev, [catId]: "Rate limited - wait 30s" })); return; }
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
  ...
  const [detectedType,    setDetectedType]   = useState("ip");
  const [loading,         setLoading]        = useState(false);
  const [copied,          setCopied]         = useState(false);
  ...
  const analyze = async () => {
    ...
    if (!detected) { setValidationError("Unrecognised format. Please enter a valid IP, domain, URL, hash, or email address."); return; }
    setIocType(detected);
  ...
  if (view === "home") return (
    <HomePageWithFeed
      setView={setView} setIocType={setIocType} setIoc={setIoc} setData={setData} onAnalyze={analyze}
      FEED_CATS={FEED_CATS} feedItems={feedItems} feedLoading={feedLoading} feedErrors={feedErrors} loadCategory={loadCategory}
    />
  );
      ...
      <div style={{ background: "#111827", borderBottom: "1px solid #1f2937", padding: "13px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 38, height: 38, borderRadius: 10, background: "linear-gradient(135deg, #1e3a5f, #3B82F6)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 12px #3B82F644" }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none"><polygon points="12,2 20,7 20,17 12,22 4,17 4,7" stroke="white" strokeWidth="1.5" fill="none"/><polygon points="12,7 16.5,9.5 16.5,14.5 12,17 7.5,14.5 7.5,9.5" stroke="#60A5FA" strokeWidth="1" fill="none"/><circle cx="12" cy="12" r="2" fill="white"/></svg>
          </div>
          <div>
            <div style={{ fontSize: 14, fontWeight: 800, color: "#F9FAFB" }}>ThreatLattice</div>
            <div style={{ fontSize: 11, color: "#4B5563" }}>Threat Intelligence Platform</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          {d && <button onClick={() => setView("report")} style={{ background: "linear-gradient(135deg,#2563EB,#3B82F6)", color: "white", border: "none", borderRadius: 8, padding: "10px 20px", fontSize: 13, fontWeight: 800, cursor: "pointer" }}>Full Report</button>}
          <button onClick={() => setView("home")} style={{ background: "#1f2937", border: "1px solid #374151", borderRadius: 8, padding: "9px 18px", fontSize: 13, color: "#9CA3AF", cursor: "pointer", fontWeight: 700 }}>Home</button>
          <button onClick={() => setView("siem")} style={{ background: "#1f2937", border: "1px solid #374151", borderRadius: 8, padding: "9px 18px", fontSize: 13, color: "#10B981", cursor: "pointer", fontWeight: 700 }}>🔌 SIEM API</button>
          <button onClick={onLogout} style={{ background: "#1f2937", border: "1px solid #374151", borderRadius: 8, padding: "9px 18px", fontSize: 13, color: "#EF4444", cursor: "pointer", fontWeight: 700 }}>Log Out</button>
        </div>
      </div>

      <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
        {pendingAnalyze && ioc && (() => { setPendingAnalyze(false); setTimeout(() => analyze(), 10); return null; })()}
        <div style={{ ...CARD }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 18 }}>
            {IOC_TYPES.map(t => (
              <button key={t.id} onClick={() => { setIocType(t.id); setIoc(""); setData(null); setValidationError(null); }}
          ...
          <div style={{ display: "flex", gap: 12 }}>
            ...
            <button onClick={analyze} disabled={loading}
              style={{ padding: "16px 36px", background: loading ? "#374151" : "linear-gradient(135deg, #2563EB, #3B82F6)", color: "white", border: "none", borderRadius: 12, fontSize: 16, fontWeight: 900, cursor: loading ? "default" : "pointer", whiteSpace: "nowrap", boxShadow: loading ? "none" : "0 0 24px #3B82F666" }}>
          ...
          {loading && <div style={{ height: 3, background: "#1f2937", borderRadius: 2, overflow: "hidden", marginTop: 14 }}><div style={{ height: 3, background: "#3B82F6", borderRadius: 2, animation: "loadbar 1.4s ease-in-out infinite", width: "40%" }}/></div>}
        </div>
          ...
          <>
            ...
            <div style={{ ...CARD, background: "#0d1b2e", border: "1px solid #3B82F633" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                <button onClick={handleCopy} style={{ fontSize: 12, padding: "8px 18px", borderRadius: 8, border: "1px solid #3B82F633", background: copied ? "#0d2d1a" : "#1e3a5f", cursor: "pointer", color: copied ? "#10B981" : "#60A5FA", fontWeight: 800 }}>{copied ? "✓ Copied" : "Copy"}</button>
              </div>
              {summaryLines.map((line, i) => (
                <div key={i} style={{ padding: "6px 0", borderBottom: i < summaryLines.length - 1 ? "1px solid #1e3a5f" : "none", fontSize: i === 0 ? 14 : 12, fontWeight: i === 0 ? 800 : 400, color: i === 0 ? "#F9FAFB" : "#60A5FA", fontFamily: "monospace" }}>{line}</div>
              ))}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
              <div style={{ ...CARD, display: "flex", flexDirection: "column", alignItems: "center" }}>
              ...
              <div style={{ ...CARD }}>
                <div style={{ ...PTITLE }}>SOURCE DISTRIBUTION</div>
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <PieChart data={Object.entries(d.sources).map(([name, src]) => { const val = src?.score ?? 0; const isClean = val <= 1; return { label: name, value: isClean ? 5 : val, color: isClean ? "#10B981" : (SOURCE_COLORS[name] || "#888") }; })} size={150} centerLabel={Object.keys(d.sources).length} centerSub="SOURCES"/>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {Object.entries(d.sources).map(([name, src]) => { const s = src?.score ?? 0; const col = SOURCE_COLORS[name] || "#888"; return (<div key={name} style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ width: 9, height: 9, borderRadius: "50%", background: s > 1 ? col : "#10B981", display: "inline-block" }}/><span style={{ fontSize: 11, color: "#6B7280" }}>{name}</span><span style={{ fontSize: 12, fontWeight: 700, color: "#F9FAFB", marginLeft: "auto" }}>{s <= 1 ? "Clean" : s}</span></div>); })}
                  </div>
                </div>
              </div>
              <div style={{ ...CARD, display: "flex", flexDirection: "column" }}>
                <div style={{ ...PTITLE }}>CONFIDENCE</div>
                <div style={{ display: "flex", justifyContent: "center", alignItems: "center", flex: 1 }}><ConfidenceDonut confidence={d.confidence} size={150}/></div>
              </div>
            </div>
            <div style={{ ...CARD }}><div style={{ ...PTITLE }}>SOURCE SCORES</div><BarChart sources={d.sources} compact={true}/></div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 14 }}>
              <div style={{ ...CARD }}>
                <div style={{ ...PTITLE }}>VERDICT &amp; SCORING</div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                  <span style={{ padding: "8px 20px", borderRadius: 28, fontSize: 14, fontWeight: 900, background: verdictStyle(d.verdict).bg, color: verdictStyle(d.verdict).color, border: `1px solid ${verdictStyle(d.verdict).border}` }}>{verdictStyle(d.verdict).label}</span>
                  <span style={{ fontSize: 34, fontWeight: 900, color: scoreColor(d.score) }}>{d.score}<span style={{ fontSize: 14, color: "#4B5563" }}>/100</span></span>
                </div>
                <div style={{ background: "#1f2937", borderRadius: 6, height: 12, overflow: "hidden", marginBottom: 8 }}><div style={{ height: 12, borderRadius: 6, width: `${d.score}%`, background: scoreColor(d.score) }}/></div>
                {[["Type", activeType?.label], ["Confidence", `${d.confidence}%`]].map(([l, v]) => (<div key={l} style={{ display: "flex", justifyContent: "space-between", padding: "9px 0", borderBottom: "1px solid #1f2937" }}><span style={{ fontSize: 12, color: "#6B7280" }}>{l}</span><span style={{ fontSize: 13, fontWeight: 700, color: "#F9FAFB" }}>{v}</span></div>))}
                <button onClick={() => setView("report")} style={{ marginTop: 16, width: "100%", padding: "13px", background: "linear-gradient(135deg, #1e3a5f, #2563EB)", color: "#60A5FA", border: "1px solid #3B82F633", borderRadius: 10, fontSize: 14, fontWeight: 800, cursor: "pointer" }}>View Full Report →</button>
              </div>
              <div style={{ ...CARD }}>
                <div style={{ ...PTITLE }}>INTELLIGENCE SOURCES</div>
                {Object.entries(d.sources).map(([name, src]) => { const s = src?.score ?? 0; const b = sourceBadge(s); const col = SOURCE_COLORS[name] || "#888"; return (<div key={name} style={{ padding: "10px 12px", border: "1px solid #1f2937", borderRadius: 10, marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between", background: "#0d1117" }}><div style={{ display: "flex", alignItems: "center", gap: 9 }}><span style={{ width: 10, height: 10, borderRadius: "50%", background: col, display: "inline-block" }}/><div><div style={{ fontSize: 13, fontWeight: 700, color: "#F9FAFB" }}>{name}</div>{src?.link && <a href={src.link} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: "#3B82F6", textDecoration: "none" }}>View →</a>}</div></div><span style={{ fontSize: 12, padding: "4px 12px", borderRadius: 20, background: b.bg, color: b.color, fontWeight: 800 }}>{s === 0 ? "Clean" : `${s}/100`}</span></div>); })}
              </div>
              <div style={{ ...CARD }}>
                <div style={{ ...PTITLE }}>ENRICHMENT &amp; PIVOT</div>
                {d.geo && Object.entries(d.geo).slice(0, 5).map(([k, v]) => (<div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #1f2937" }}><span style={{ fontSize: 12, color: "#6B7280" }}>{k}</span><span style={{ fontSize: 13, fontWeight: 700, color: "#F9FAFB" }}>{String(v)}</span></div>))}
                <div style={{ marginTop: 14 }}>{dashLinks.slice(0, 6).map(([label, url]) => (<a key={label} href={url} target="_blank" rel="noreferrer" style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: "#3B82F6", padding: "8px 0", borderBottom: "1px solid #1f2937", textDecoration: "none", fontWeight: 600 }}>{label}<span style={{ color: "#374151", fontSize: 11 }}>→</span></a>))}</div>
              </div>
            </div>
          </>
        )}
...
function PasswordStrengthBar({ password }) {
  ...
  const score = (() => { let s = 0; if (password.length >= 12) s++; if (password.length >= 16) s++; if (/[A-Z]/.test(password)) s += 0.5; if (/[a-z]/.test(password)) s += 0.5; if (/\d/.test(password)) s += 0.5; if (/[!@#$%^&*()\-_+=\[\]{}|;:,.<>?]/.test(password)) s += 0.5; return Math.min(4, Math.floor(s)); })();
  const labels = ["Very Weak", "Weak", "Medium", "Strong", "Very Strong"];
  const colors = ["#EF4444", "#F97316", "#F59E0B", "#10B981", "#10B981"];
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ display: "flex", gap: 4, marginBottom: 4 }}>{[0,1,2,3].map(i => (<div key={i} style={{ flex: 1, height: 4, borderRadius: 2, background: i < score ? colors[score] : "#1f2937", transition: "background .3s" }}/>))}</div>
      <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontSize: 11, color: colors[score], fontWeight: 700 }}>{labels[score]}</span></div>
    </div>
  );
}
      ...
      <div style={{ position: "relative", width: "100%", maxWidth: 420 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ width: 56, height: 56, borderRadius: 16, background: "linear-gradient(135deg, #1e3a5f, #3B82F6)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", boxShadow: "0 0 32px #3B82F655" }}>
            <svg width="26" height="26" viewBox="0 0 24 24" fill="none"><polygon points="12,2 20,7 20,17 12,22 4,17 4,7" stroke="white" strokeWidth="1.5" fill="none"/><polygon points="12,7 16.5,9.5 16.5,14.5 12,17 7.5,14.5 7.5,9.5" stroke="#60A5FA" strokeWidth="1" fill="none"/><circle cx="12" cy="12" r="2" fill="white"/></svg>
          </div>
          <div style={{ fontSize: 10, fontWeight: 800, color: "#3B82F6", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 8 }}>ThreatLattice</div>
        ...
        <div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 18, padding: "32px" }}>{children}</div>
      </div>
    </div>
  );
}
...
function AuthInput({ label, type = "text", value, onChange, placeholder, autoFocus }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#9CA3AF", marginBottom: 6 }}>{label}</label>
...
function AuthBtn({ children, onClick, loading, disabled, variant = "primary" }) {
  const bg = variant === "primary" ? (disabled || loading ? "#1f2937" : "linear-gradient(135deg, #2563EB, #3B82F6)") : "#1f2937";
  return (
    <button onClick={onClick} disabled={disabled || loading}
      style={{ width: "100%", padding: "13px", background: bg, border: variant === "secondary" ? "1px solid #374151" : "none", borderRadius: 10, fontSize: 14, color: disabled || loading ? "#4B5563" : "white", fontWeight: 800, cursor: disabled || loading ? "default" : "pointer" }}>
...
function RegisterScreen({ onSwitch }) {
  const [email, setEmail] = useState(""); const [password, setPassword] = useState(""); const [confirm, setConfirm] = useState(""); const [loading, setLoading] = useState(false); const [error, setError] = useState(""); const [done, setDone] = useState(false);
  const submit = async () => {
    setError(""); if (!email || !password) return setError("Please fill in all fields."); if (password !== confirm) return setError("Passwords do not match.");
    setLoading(true);
    try {
      ...
      if (!r.ok) { const msg = d.detail?.message || d.detail || "Registration failed"; const issues = d.detail?.issues || []; setError(issues.length ? `${msg}: ${issues.join(", ")}` : msg); } else setDone(true);
    } catch { setError("Network error"); } finally { setLoading(false); }
  };
  if (done) return (<AuthCard title="Check Your Email" subtitle="A 6-digit code has been sent"><AuthSuccess msg={`Verification code sent to ${email}`}/><AuthBtn onClick={() => onSwitch("verify", email)}>Enter Verification Code →</AuthBtn><div style={{ textAlign: "center", marginTop: 16 }}><button onClick={() => onSwitch("login")} style={{ background: "none", border: "none", color: "#4B5563", fontSize: 13, cursor: "pointer" }}>Back to Login</button></div></AuthCard>);
  return (<AuthCard title="Create Account" subtitle="Join ThreatLattice"><AuthError msg={error}/><AuthInput label="Email Address" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="analyst@company.com" autoFocus/><div style={{ marginBottom: 16 }}><label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#9CA3AF", marginBottom: 6 }}>Password</label><input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Min. 12 characters" style={{ width: "100%", padding: "12px 14px", background: "#1f2937", border: "1px solid #374151", borderRadius: 10, fontSize: 14, color: "#F9FAFB", outline: "none", boxSizing: "border-box" }} onFocus={e => e.target.style.borderColor = "#3B82F6"} onBlur={e => e.target.style.borderColor = "#374151"}/><PasswordStrengthBar password={password}/></div><AuthInput label="Confirm Password" type="password" value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="Re-enter password"/><AuthBtn onClick={submit} loading={loading} disabled={!email || !password || !confirm}>Create Account</AuthBtn><div style={{ textAlign: "center", marginTop: 16 }}><span style={{ fontSize: 13, color: "#4B5563" }}>Already have an account? </span><button onClick={() => onSwitch("login")} style={{ background: "none", border: "none", color: "#3B82F6", fontSize: 13, cursor: "pointer", fontWeight: 700 }}>Sign In</button></div></AuthCard>);
}

function VerifyEmailScreen({ initialEmail, onSwitch }) {
  const [email, setEmail] = useState(initialEmail || ""); const [code, setCode] = useState(""); const [loading, setLoading] = useState(false); const [error, setError] = useState(""); const [success, setSuccess] = useState("");
  const submit = async () => {
    setError(""); setSuccess(""); if (!email || !code) return setError("Please enter your email and code.");
    setLoading(true);
    try { const r = await fetch(`${API_BASE}/auth/verify-email`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, code }) }); const d = await r.json(); if (!r.ok) { setError(typeof d.detail === "string" ? d.detail : "Verification failed"); } else { setSuccess(d.message); setTimeout(() => onSwitch("login"), 1500); } } catch { setError("Network error"); } finally { setLoading(false); }
  };
  return (<AuthCard title="Verify Your Email" subtitle="Enter the 6-digit code we sent you"><AuthError msg={error}/><AuthSuccess msg={success}/><AuthInput label="Email Address" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="analyst@company.com"/><AuthInput label="Verification Code" value={code} onChange={e => setCode(e.target.value.replace(/\D/g,"").slice(0,6))} placeholder="123456" autoFocus/><AuthBtn onClick={submit} loading={loading} disabled={code.length !== 6}>Verify Email</AuthBtn><div style={{ textAlign: "center", marginTop: 12 }}><button onClick={() => onSwitch("login")} style={{ background: "none", border: "none", color: "#4B5563", fontSize: 12, cursor: "pointer" }}>Back to Login</button></div></AuthCard>);
}
...
function LoginScreen({ onSwitch, onMFA }) {
  ...
  const submit = async () => {
    setError(""); if (!email || !password) return setError("Please enter your email and password."); setLoading(true);
    try {
      ...
      const d = await r.json();
      if (!r.ok) { let msg = "Login failed. Please try again."; if (typeof d.detail === "string") msg = d.detail; else if (Array.isArray(d.detail)) msg = "Invalid request - please check your input."; else if (d.detail?.message) msg = d.detail.message; setError(msg); } else onMFA(email);
    } catch (e) { setError("Network error - is the server running?"); } finally { setLoading(false); }
  };
  return (<AuthCard title="Welcome Back" subtitle="Sign in to your analyst account"><AuthError msg={error}/><AuthInput label="Email Address" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="analyst@company.com" autoFocus/><AuthInput label="Password" type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Your password"/><div style={{ textAlign: "right", marginBottom: 20, marginTop: -8 }}><button onClick={() => onSwitch("forgot")} style={{ background: "none", border: "none", color: "#4B5563", fontSize: 12, cursor: "pointer" }}>Forgot password?</button></div><AuthBtn onClick={submit} loading={loading} disabled={!email || !password}>Sign In</AuthBtn><div style={{ textAlign: "center", marginTop: 16 }}><span style={{ fontSize: 13, color: "#4B5563" }}>No account? </span><button onClick={() => onSwitch("register")} style={{ background: "none", border: "none", color: "#3B82F6", fontSize: 13, cursor: "pointer", fontWeight: 700 }}>Create one</button></div></AuthCard>);
}
...
function MFAScreen({ email, onAuth, onSwitch }) {
  ...
  const submit = async () => {
    setError(""); if (code.length !== 6) return setError("Enter the 6-digit code from your email."); setLoading(true);
    try { const r = await fetch(`${API_BASE}/auth/verify-mfa`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, code }) }); const d = await r.json(); if (!r.ok) { setError(typeof d.detail === "string" ? d.detail : "Invalid code"); } else { setToken(d.access_token); onAuth(); } } catch { setError("Network error"); } finally { setLoading(false); }
  };
  return (<AuthCard title="Two-Factor Verification" subtitle={`Code sent to ${email}`}><AuthError msg={error}/><AuthInput label="Security Code" value={code} onChange={e => setCode(e.target.value.replace(/\D/g,"").slice(0,6))} placeholder="123456" autoFocus/><AuthBtn onClick={submit} loading={loading} disabled={code.length !== 6}>Verify & Sign In</AuthBtn><div style={{ textAlign: "center", marginTop: 12 }}><button onClick={() => onSwitch("login")} style={{ background: "none", border: "none", color: "#4B5563", fontSize: 12, cursor: "pointer" }}>← Back to Login</button></div></AuthCard>);
}
...
function ForgotPasswordScreen({ onSwitch }) {
  ...
  const submit = async () => {
    setError(""); if (!email) return setError("Please enter your email."); setLoading(true);
    try { const r = await fetch(`${API_BASE}/auth/forgot-password`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email }) }); const d = await r.json(); if (!r.ok) { setError(typeof d.detail === "string" ? d.detail : "Request failed - please try again."); } else setSent(true); } catch { setError("Network error"); } finally { setLoading(false); }
  };
  if (sent) return (<AuthCard title="Check Your Email" subtitle="Password reset link sent"><AuthSuccess msg="If that email is registered, a reset link has been sent."/><AuthBtn onClick={() => onSwitch("login")} variant="secondary">← Back to Login</AuthBtn></AuthCard>);
  return (<AuthCard title="Reset Password" subtitle="We'll send a reset link to your email"><AuthError msg={error}/><AuthInput label="Email Address" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="analyst@company.com" autoFocus/><AuthBtn onClick={submit} loading={loading} disabled={!email}>Send Reset Link</AuthBtn><div style={{ textAlign: "center", marginTop: 12 }}><button onClick={() => onSwitch("login")} style={{ background: "none", border: "none", color: "#4B5563", fontSize: 13, cursor: "pointer" }}>← Back to Login</button></div></AuthCard>);
}
...
function ResetPasswordScreen({ token, onSwitch }) {
  ...
  const submit = async () => {
    setError(""); if (!password || !confirm) return setError("Please fill in both fields."); if (password !== confirm) return setError("Passwords do not match."); setLoading(true);
    try {
      const r = await fetch(`${API_BASE}/auth/reset-password`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token, new_password: password }) });
      const d = await r.json();
      if (!r.ok) { let msg = "Reset failed - please try again."; if (typeof d.detail === "string") msg = d.detail; else if (d.detail?.message) { const issues = d.detail.issues || []; msg = issues.length ? `${d.detail.message}: ${issues.join(", ")}` : d.detail.message; } setError(msg); } else setDone(true);
    } catch { setError("Network error"); } finally { setLoading(false); }
  };
  if (done) return (<AuthCard title="Password Updated" subtitle="Your password has been changed"><AuthSuccess msg="Password updated successfully!"/><AuthBtn onClick={() => onSwitch("login")}>Sign In →</AuthBtn></AuthCard>);
  return (<AuthCard title="Set New Password" subtitle="Choose a strong password"><AuthError msg={error}/><div style={{ marginBottom: 16 }}><label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#9CA3AF", marginBottom: 6 }}>New Password</label><input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Min. 12 characters" autoFocus style={{ width: "100%", padding: "12px 14px", background: "#1f2937", border: "1px solid #374151", borderRadius: 10, fontSize: 14, color: "#F9FAFB", outline: "none", boxSizing: "border-box" }} onFocus={e => e.target.style.borderColor = "#3B82F6"} onBlur={e => e.target.style.borderColor = "#374151"}/><PasswordStrengthBar password={password}/></div><AuthInput label="Confirm New Password" type="password" value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="Re-enter password"/><AuthBtn onClick={submit} loading={loading} disabled={!password || !confirm}>Update Password</AuthBtn></AuthCard>);
}
...
function AuthGate({ onAuth }) {
  ...
  const switchTo = (s, email = "") => { setScreen(s); if (email) { setMfaEmail(email); setVerifyEmail(email); } };
...
function SiemKeysPage({ onBack }) {
  const [keys, setKeys] = useState([]); const [loading, setLoading] = useState(true); const [creating, setCreating] = useState(false); const [newName, setNewName] = useState(""); const [newKey, setNewKey] = useState(null); const [error, setError] = useState(""); const [copied, setCopied] = useState(false);
  const fetchKeys = async () => { try { const r = await fetch(`${API_BASE}/siem/keys`, { headers: authHeaders() }); if (r.ok) setKeys(await r.json()); } catch(e) { setError("Failed to load keys"); } finally { setLoading(false); } };
  useEffect(() => { fetchKeys(); }, []);
  const createKey = async () => { if (!newName.trim()) return; setCreating(true); setError(""); try { const r = await fetch(`${API_BASE}/siem/keys`, { method: "POST", headers: { "Content-Type": "application/json", ...authHeaders() }, body: JSON.stringify({ name: newName.trim() }) }); const d = await r.json(); if (!r.ok) { setError(d.detail || "Failed to create key"); return; } setNewKey(d); setNewName(""); fetchKeys(); } catch(e) { setError("Network error"); } finally { setCreating(false); } };
  const revokeKey = async (id) => { if (!confirm("Revoke this API key? This cannot be undone.")) return; try { await fetch(`${API_BASE}/siem/keys/${id}`, { method: "DELETE", headers: authHeaders() }); fetchKeys(); } catch(e) { setError("Failed to revoke key"); } };
  const copyKey = () => { navigator.clipboard.writeText(newKey.raw_key); setCopied(true); setTimeout(() => setCopied(false), 2000); };
  return (
    <div style={{ fontFamily: "system-ui, sans-serif", background: "#0b0f14", minHeight: "100vh", color: "#e0eaf4" }}>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "32px 24px" }}>
        <div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 14, padding: "20px 24px", marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 13, color: "#6B7280", lineHeight: 1.8, marginBottom: 16 }}>Generate an API key below and use it to submit IOCs programmatically from Splunk, Elastic SIEM, Microsoft Sentinel, IBM QRadar, or any REST-capable SOAR platform. Up to 10 IOCs per request, 60 requests/minute per key.</div>
          <div style={{ background: "#0d1117", borderRadius: 8, padding: "12px 16px", fontFamily: "monospace", fontSize: 12, color: "#10B981", lineHeight: 2 }}>
            ...
            <div>{"  "}-d '{`{"iocs": ["185.220.101.35"]}`}'</div>
          </div>
        </div>
        {newKey && (<div style={{ background: "#0a2e1a", border: "1px solid #065f26", borderRadius: 12, padding: "20px 24px", marginBottom: 24 }}><div style={{ fontSize: 14, fontWeight: 700, color: "#34d399", marginBottom: 8 }}>✓ API Key Created - Copy it now</div><div style={{ fontSize: 12, color: "#6B7280", marginBottom: 12 }}>This key will not be shown again.</div><div style={{ display: "flex", gap: 10, alignItems: "center" }}><div style={{ flex: 1, background: "#111827", border: "1px solid #1f2937", borderRadius: 8, padding: "10px 14px", fontFamily: "monospace", fontSize: 13, color: "#34d399", wordBreak: "break-all" }}>{newKey.raw_key}</div><button onClick={copyKey} style={{ padding: "10px 18px", background: copied ? "#065f26" : "#10B981", border: "none", borderRadius: 8, color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>{copied ? "Copied!" : "Copy"}</button></div><button onClick={() => setNewKey(null)} style={{ marginTop: 12, background: "none", border: "none", color: "#4B5563", fontSize: 12, cursor: "pointer" }}>Dismiss</button></div>)}
        <div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 14, padding: "20px 24px", marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <input value={newName} onChange={e => setNewName(e.target.value)} placeholder='e.g. "Splunk Production"' onKeyDown={e => e.key === "Enter" && createKey()} style={{ flex: 1, background: "#1f2937", border: "1px solid #374151", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "#F9FAFB", outline: "none" }}/>
            <button onClick={createKey} disabled={creating || !newName.trim()} style={{ padding: "10px 24px", background: creating || !newName.trim() ? "#1f2937" : "linear-gradient(135deg,#2563EB,#3B82F6)", border: "none", borderRadius: 8, color: creating || !newName.trim() ? "#4B5563" : "#fff", fontWeight: 700, fontSize: 13, cursor: creating || !newName.trim() ? "default" : "pointer" }}>{creating ? "Creating..." : "Generate Key"}</button>
          </div>
        </div>
        <div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 14, overflow: "hidden" }}>
          <div style={{ padding: "16px 24px", borderBottom: "1px solid #1f2937", fontSize: 14, fontWeight: 700, color: "#F9FAFB" }}>Active API Keys ({keys.length})</div>
          {loading ? <div style={{ padding: "32px", textAlign: "center", color: "#4B5563" }}>Loading...</div> : keys.length === 0 ? <div style={{ padding: "32px", textAlign: "center", color: "#4B5563", fontSize: 13 }}>No API keys yet.</div> : keys.map(k => (<div key={k.id} style={{ padding: "16px 24px", borderBottom: "1px solid #111827", display: "flex", alignItems: "center", gap: 16 }}><div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 700, color: "#F9FAFB", marginBottom: 4 }}>{k.name}</div><div style={{ fontSize: 12, color: "#4B5563", fontFamily: "monospace" }}>{k.key_prefix}•••••••••••••••••<span style={{ marginLeft: 16 }}>Created {new Date(k.created_at).toLocaleDateString()}</span><span style={{ marginLeft: 16, color: "#3B82F6" }}>{k.requests_total} requests</span></div></div><button onClick={() => revokeKey(k.id)} style={{ padding: "6px 14px", background: "transparent", border: "1px solid #7f1d1d", borderRadius: 6, color: "#EF4444", fontSize: 12, cursor: "pointer", fontWeight: 700 }}>Revoke</button></div>))}
        </div>
        <div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 14, padding: "20px 24px", marginTop: 24 }}>
          ...
          {[["Splunk","#3B82F6","REST API Input or Adaptive Response Action"],["Elastic SIEM","#F59E0B","Elastic Watcher or Custom Connector on rule trigger"],["Microsoft Sentinel","#10B981","Logic App with HTTP Action from Automation Rule"],["IBM QRadar","#A78BFA","Custom Action script or SOAR integration"]].map(([name,color,desc]) => (<div key={name} style={{ display: "flex", gap: 12, marginBottom: 12, padding: "12px 14px", background: "#0d1117", borderRadius: 8, borderLeft: `3px solid ${color}` }}><div style={{ minWidth: 120, fontWeight: 700, color, fontSize: 13 }}>{name}</div><div style={{ fontSize: 13, color: "#6B7280" }}>{desc}</div></div>))}
        </div>
      </div>
    </div>
  );
